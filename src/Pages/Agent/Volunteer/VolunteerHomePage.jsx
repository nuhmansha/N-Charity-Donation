...
const VolunteerHomePage = () => {
  ...
  const renderForm = () => {
    switch (currentView) {
      case "event":
        return <EventForm addEvent={handleEventSubmit} />;
      case "cause":
        return <CauseForm addCause={handleCauseSubmit} />;
      case "blog":
        return <BlogForm addBlog={handleBlogSubmit} />;
      default:
        return null;
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-r from-purple-400 via-pink-500 to-red-500">
      <header className="p-4 flex justify-between items-center bg-white shadow-lg">
        ...
        <div>
          <button className="px-4 py-2 bg-red-400 text-white rounded-lg shadow-md hover:bg-red-600 transition duration-300">
            Logout
          </button>
        </div>
      </header>
      <div className="flex flex-col md:flex-row">
        <aside className="w-full md:w-64 p-4 bg-white shadow-lg">
          <ul className="space-y-4">
            <li
              className={`flex items-center p-4 rounded-lg cursor-pointer transition duration-300 ${currentView === "event" ? "bg-blue-500 text-white" : "bg-gray-100"}`}
              onClick={() => {
            ...
            <li
              className={`flex items-center p-4 rounded-lg cursor-pointer transition duration-300 ${currentView === "cause" ? "bg-green-500 text-white" : "bg-gray-100"}`}
              onClick={() => {
                setCurrentView("cause");
                setFormVisible(false);
              }}
            >
              <span className="w-6 h-6 mr-2 bg-green-500 rounded-full"></span>
            ...
            <li
              className={`flex items-center p-4 rounded-lg cursor-pointer transition duration-300 ${currentView === "blog" ? "bg-red-500 text-white" : "bg-gray-100"}`}
              onClick={() => {
                setCurrentView("blog");
                setFormVisible(false);
              }}
        ...
        <main className="flex-1 p-6 bg-white shadow-lg rounded-lg">
          <div className="flex justify-end mb-4">
            ...
            >
              {formVisible ? "Hide Form" : `Add New ${currentView.charAt(0).toUpperCase() + currentView.slice(1)}`}
            </button>
          </div>
          {formVisible && (
          ...
          <hr className="my-8" />
          <h2 className="text-2xl font-semibold mb-4">{`${currentView.charAt(0).toUpperCase() + currentView.slice(1)}s`}</h2>
          <DataTable
            data={currentView === "event" ? events : currentView === "cause" ? causes : blogs}
            columns={currentView === "event" ? eventColumns : currentView === "cause" ? causeColumns : blogColumns}
            onEdit={handleEdit}
            onDelete={
              currentView === "event"
                ? handleDeleteEvent
